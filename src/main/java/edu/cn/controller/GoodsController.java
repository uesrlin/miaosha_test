...
import edu.cn.vo.GoodsDetailVo;
...
//手动渲染
    ...
    @ApiOperation(value = "秒杀的商品详情")
    @RequestMapping(value="/to_detail1/{goodsId}", produces = "text/html")
    @ResponseBody
    public String detail1(HttpServletRequest request, HttpServletResponse response, Model model, MsUser user, @PathVariable("goodsId") long goodsId) {
        model.addAttribute("user", user);
        ...
        long startAt = goods.getStartDate().getTime();  //转为毫秒
        long endAt = goods.getEndDate().getTime();
        long now = System.currentTimeMillis();

        int miaoshaStatus = 0;  //秒杀状态
        int remainSeconds = 0;  //距离开始秒杀时间还有多久
    ...
    @ResponseBody
    public Result<GoodsDetailVo> detail(HttpServletRequest request, HttpServletResponse response, Model model, MsUser msuser, @PathVariable("goodsId") long goodsId) {
        GoodsVo goods = goodsService.getGoodsVoByGoodsId(goodsId);
        ...
        goodsDetailVo.setMsUser(msuser);
