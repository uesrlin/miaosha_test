...
public class UserArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public boolean supportsParameter(MethodParameter methodParameter) {
        ...
        return clazz == MsUser.class;      //如果为真，才会执行下面的resolveArguement方法
    }
    ...
    public Object resolveArgument(MethodParameter methodParameter, ModelAndViewContainer modelAndViewContainer,
        ...
        String paramToken = request.getParameter(MsService.COOKIE_NAME_TOKEN);         //再拿到paramToken和cookieToken
        String cookieToken = getCookieValue(request, MsService.COOKIE_NAME_TOKEN);
        if (StringUtils.isEmpty(cookieToken) && StringUtils.isEmpty(paramToken)){
            return null;
        }
        String token = StringUtils.isEmpty(paramToken)?cookieToken:paramToken;  //paramToken优先使用
        return msService.getByToken(response, token);
    }
